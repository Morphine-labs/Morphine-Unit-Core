...
@external
func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(factory: felt, proxy_admin: felt) {
...
// @return amounts The input token amount and all subsequent output token amounts
@external
func swap_tokens_for_exact_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    amountOut: Uint256, amountInMax: Uint256, path_len: felt, path: felt*, to: felt, deadline: felt
) -> (amounts_len: felt, amounts: Uint256*) {
    alloc_locals;
    _ensure_deadline(deadline);
    let (local factory) = _factory.read();
...
func _add_liquidity{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (local reserveA: Uint256, local reserveB: Uint256) = _get_reserves(factory, tokenA, tokenB);
