...
@storage_var
func pool_factory() -> (pool_factory_address: felt) {
}
...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    _pool_factory: felt, _eth_address: felt
) {
...
func only_authorized{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() {
    ...
    let (owner_) = IPoolFactory.getOwner(pool_factory_);
    with_attr error_message("onlyAuthorized: only callable by the owner or PF") {
        assert (owner_ - caller_) * (pool_factory_ - caller_) = 0;
    }
    return ();
}
...
func calculAssetValue{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    let (pool_factory_ : felt) = pool_factory.read();
    let (primitivePriceFeed_: felt) = IPoolFactory.getPrimitivePriceFeed(vault_factory_);
    let (
