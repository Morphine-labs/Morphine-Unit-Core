...
@external
func addPrimitive{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(_token: felt, pair_id: felt){
    configurator_only();
...
@external
func addDerivative{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(_token: felt, _derivative_price_feed: felt){
    configurator_only();
    with_attr error_message("zero address"){
    ...
    derivative_price_feed.write(_token, _derivative_price_feed);
...
func derivativePriceFeed{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _derivative: felt) -> (price_feed: felt) {
    let (price_feed_: felt) = derivative_price_feed.read(_derivative);
    return (price_feed_,);
}
...
func convertToUSD{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _amount: Uint256
    _token: felt) -> (tokenPriceUsd: Uint256) {
    let (decimals_) = IERC20.decimals(_token);
...
func convertFromUSD{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _amount: Uint256
    _token: felt) -> (tokenPrice: Uint256) {
    let (decimals_) = IERC20.decimals(_token);
    let (multiplier_) = pow(10, decimals_);
    let (token_price_) = get_price(_token);
...
func fastCheck{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    _token_from: felt,
    _amount_to: Uint256,
    _token_to: felt) -> (collateralFrom: Uint256, collateralTo: Uint256) {
...
func get_price{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _token: felt) -> (token_price_usd: Uint256) {
    ...
    if(derivative_to_price_feed_ == 0){
        ...
        let (underlying_assets_len: felt, underlying_assets: felt*, underlyings_amounts_len: felt, underlyings_amounts: Uint256*) = IDerivativePriceFeed.calcUnderlyingValues(derivative_to_price_feed_, _token);
        with_attr error_message("quote price error"){
            assert_not_zero(underlying_assets_len);
        }
        let (price_) = recursive_calcul_derivative(underlying_assets_len, underlying_assets, underlyings_amounts_len, underlyings_amounts);
        return(price_,);
    }
    return (token_price_usd_,);
}
...
func recursive_calcul_derivative{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    underlying_amounts: Uint256*) -> (res: Uint256) {
    alloc_locals;
