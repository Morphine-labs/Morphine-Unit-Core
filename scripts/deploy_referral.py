...
async def deploy():
    ...
    print(f'⌛️ Declaring ref...')
    declare_transaction_ref = await admin.sign_declare_transaction(
    compilation_source=Path(utils.REF_SOURCE_CODE).read_text(), max_fee=int(1e16)
    )
    resp = await admin.declare(transaction=declare_transaction_ref)
    await admin.wait_for_tx(resp.transaction_hash)
    ref_class_hash = resp.class_hash

    print(f'✅ Success! Class Hash: {ref_class_hash} ')
    
    print(f'⌛️ Deploying referral...')
    deploy_ref_call, ref = deployer.create_deployment_call(
    class_hash=ref_class_hash)
    resp = await admin.execute(deploy_ref_call, max_fee=int(1e16))
    await admin.wait_for_tx(resp.transaction_hash)
    print(f'✅ Success! Data Provider deployed to {ref} ')
