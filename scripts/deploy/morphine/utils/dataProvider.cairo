...
@view
func getFaucetInfo{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(_user: felt, faucet_array_len: felt, faucet: felt*) -> (
    faucetInfo_len: felt, faucetInfo: FaucetInfo*
) {
    let (faucet_info: FaucetInfo*) = alloc();
    recursive_faucet_info(_user, faucet_array_len, faucet, faucet_info);
    return (faucet_array_len, faucet_info,);
}


func recursive_faucet_info{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(_user: felt, faucet_array_len: felt, faucet: felt*, faucet_info: FaucetInfo*) {
    if(faucet_array_len == 0){
        return();
    }
    let (token_address_) = IFaucet.get_token_address(faucet[0]);
    ...
    let (state_) = IFaucet.isAllowedForTransaction(faucet[0], _user);

    if(state_ == 1){
        assert faucet_info[0].remaining_time = 0;
        return recursive_faucet_info(_user, faucet_array_len - 1, faucet + 1, faucet_info + FaucetInfo.SIZE);
    }   else {
        let (allowed_time_) = IFaucet.get_allowed_time(faucet[0], _user);
