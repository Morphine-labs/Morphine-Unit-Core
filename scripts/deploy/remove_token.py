...
async def call():
    ...
    print(f'⌛️ forbidding token...')
    drip_configurator_contract = await Contract.from_address(client=admin, address=utils.DAI_DRIP_CONFIGURATOR)
    invocation = await drip_configurator_contract.functions["setLiquidationThreshold"].invoke(TOKEN_ADDRESS, LT_TOKEN, max_fee=int(1e16))
