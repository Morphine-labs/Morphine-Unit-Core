...
async def deploy():
    ...
    print(f'⌛️ Declaring erc4626 price feed...')
    declare_transaction_erc4626_price_feed = await admin.sign_declare_transaction(
    compilation_source=Path(utils.ERC4626_PRICE_FEED_SOURCE_CODE).read_text(), max_fee=int(1e16)
    )

    resp = await admin.declare(transaction=declare_transaction_erc4626_price_feed)
    await admin.wait_for_tx(resp.transaction_hash)
    erc4626_price_feed_class_hash = resp.class_hash
    print(f'✅ Success! Class Hash: {erc4626_price_feed_class_hash} ')
    

    print(f'⌛️ Deploying erc4626 price feed...')
    deploy_erc4626_price_feed_call, erc4626_price_feed = deployer.create_deployment_call(
    class_hash=erc4626_price_feed_class_hash)
    resp = await admin.execute(deploy_erc4626_price_feed_call, max_fee=int(1e16))
    await admin.wait_for_tx(resp.transaction_hash)
    print(f'✅ Success! ERC4626 PRICEFEED deployed to {erc4626_price_feed} ')
